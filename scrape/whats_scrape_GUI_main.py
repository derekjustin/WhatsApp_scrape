...
def main():
    ...
    while True:
        ...
        if event == 'Quit':
            ...
            window.finalize()
            if group_tools.save_all_groups_data() is False:
                window = gui.make_save_all_groups_data_fail_screen(window)
                event, values = window.read()
                if event == 'Return':
                    window.close()
                    window = gui.make_home_menu()
            else:
                window = gui.make_save_all_groups_data_success_screen(window)
                event, values = window.read()
                if event == 'Return':
                    window.close()
                    window = gui.make_home_menu()